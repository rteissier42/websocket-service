...
export class AppConfigService implements IAppConfigService {
  ...
  constructor(private http: HttpBackend, private loggerService: LoggerService) {}
  ...
  public load(): Observable<IAppConf> {
    if (!this.configuration$) {
      this.configuration$ = new HttpClient(this.http).get<IAppConf>(this.configUrl).pipe(
        /**
  ...
  private isValidConfigUrl(url: unknown): url is ConfigUrl {
    return typeof url === 'string' && Object.values(ConfigUrl).includes(url as ConfigUrl);
  }
}
