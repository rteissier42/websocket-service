...
module util.webSocket {
  export interface IWebSocketService {
    subscribe(
      url: string,
      handler: (message: StompMessage) => any,
      keepActive?: boolean
    )
  ...
  export class WebSocketSuscriber {
    constructor(
      public url: string,
      public handler: (message: StompMessage) => any,
  ...
  export class WebSocketService implements IWebSocketService {
    ...
    static $inject = ['appConf', '$rootScope', '$log', '$timeout']
    ...
    private initWebSocketClient(): void {
      if (!WebSocketService.socket) {
        ...
        WebSocketService.socket = new SockJS(
          this.appConf.webSocketEndpointUrl.toString()
        )
    ...
    subscribe(
      ...
      //CONNECTING
        ...
        WebSocketService.subscribers.push(
          new WebSocketSuscriber(url, handler, keepActive)
        )
      } else {
        return false
      }
    ...
    clearSubscriptions(): void {
      WebSocketService.subscriptions.forEach((sub: Subscription) =>
        sub.unsubscribe()
      )
    }
